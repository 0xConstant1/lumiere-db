...
LIMIT $5`, typeList, genre, yearFrom, yearTo, limit)
		...
		results := make([]DiscoverItem, 0, limit)
		for rows.Next() {
			var (
				tconst        string
				ttype         string
			...
			if err := rows.Scan(&tconst, &ttype, &primaryTitle, &originalTitle, &startYear, &endYear, &genres, &avgRating, &numVotes); err != nil {
				return err
			}
			if genres == nil {
				genres = []string{}
			}
			results = append(results, DiscoverItem{
				Tconst:        tconst,
				TitleType:     ttype,
				PrimaryTitle:  primaryTitle,
				OriginalTitle: originalTitle,
